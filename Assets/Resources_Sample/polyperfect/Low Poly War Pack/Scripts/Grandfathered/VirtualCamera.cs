                        ...
                        if (isActive)
                        {
                            ...
                            else
                            {
                                ...
                                if (hitNumber > 0)
                                {
                                    ...
                                    {
                                        if (!(raycastHits[i].collider is CharacterController)&&!raycastHits[i].transform.IsChildOf(followTarget.transform.parent?followTarget.transform.parent:followTarget.transform) && raycastHits[i].distance > cameraColliderRadius)
                                        {
                                ...
                                if (!triggered)
                                {
                                    ...
                                    transform.position = Vector3.MoveTowards(transform.position, followTarget.position + (transform.position - followTarget.position) * (distance / (Mathf.Max(Vector3.Distance(transform.position, followTarget.position),1f))), 0.4f);
                                    cameraTransform.position = transform.position;
                                }
